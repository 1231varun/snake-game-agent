...
def index():
    ...
    return render_template("game_play.html", mode=game_state["mode"], model_path=game_state["model_path"] or "None")
...
def handle_error(e):
    ...
    if request.path.startswith("/api/") or request.path == "/get_game_state" or request.path == "/get_frame":
        return (
            jsonify({"success": False, "error": str(e), "status": "error", "status_text": f"Server error: {str(e)}"}),
            500,
        )
