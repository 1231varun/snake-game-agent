...
def add_log_message(message):
    ...
    training_state["log_messages"].append({
...
class VisualTrainer(SnakeTrainer):
        ...
        if info:
            ...
            if info.get('done', False):
                if info.get('timeout', False):
                    ...
                    add_log_message(f"Episode {episode+1} ended - Snake collision. Final score: {info.get('score', 0)}")
    ...
    def train(self):
        ...
        progress_bar = None  # No tqdm in visual mode
        ...
        for e in range(self.episodes):
            ...
            if (e+1) % print_freq == 0 or (e+1) == self.episodes:
                ...
                log_message = template.format(e+1, self.episodes, info["score"], avg_score, self.agent.epsilon, self.timeout_count)
                print(log_message)
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='models/visual_model.h5',
                        help='Path to save the trained model')
    parser.add_argument('--render-freq', type=int, default=1,
                        help='Frequency of rendering during training (0 for no rendering)')
    parser.add_argument('--save-freq', type=int, default=max(5, default_episodes // 10),
